...
class XORsolver_Adam{
    ...
    public:
        ...
        input_layer(in, h1, &opt1, init), hidden1_layer(h1, h2, &opt2, init), output_layer(h2, out, &opt3, init),
        inAct(h1, 1, act), hidden1Act(h2, 1, act), outputAct(out, 1, act), loss(out, 1, l),
        ...
        void backprop(int epochs){
            ...
            for(int epoch = 1; epoch <= epochs; epoch++){
                ...
                for(int i = 0; i < 4; i++){
                    ...
                    output_layer.backprop(nullptr, loss.getGrad(), outputAct.d_Active(out));
